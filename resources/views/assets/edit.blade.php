                    ...
                    <form action="{{ route('assets.update', $asset->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="asset_category_id" id="asset_category_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    @foreach($assetCategories as $category)
                                        <option value="{{ $category->id }}" {{ $asset->asset_category_id == $category->id ? 'selected' : '' }}>{{ $category->description }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="asset_sub_category_id" id="asset_sub_category_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    @foreach($assetSubCategories as $subCategory)
                                        <option value="{{ $subCategory->id }}" {{ $asset->asset_sub_category_id == $subCategory->id ? 'selected' : '' }}>{{ $subCategory->description }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="location_id" id="location_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    @foreach($locations as $location)
                                        <option value="{{ $location->id }}" {{ $asset->location_id == $location->id ? 'selected' : '' }}>{{ $location->location_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select name="assembly_id" id="assembly_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    ...
                                    @foreach($assemblies as $assembly)
                                        <option value="{{ $assembly->id }}" {{ $asset->assembly_id == $assembly->id ? 'selected' : '' }}>{{ $assembly->description }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <x-text-input id="purchase_date" class="block mt-1 w-full" type="date" name="purchase_date" :value="old('purchase_date', $asset->purchase_date)" />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <x-text-input id="warranty_expiration_date" class="block mt-1 w-full" type="date" name="warranty_expiration_date" :value="old('warranty_expiration_date', $asset->warranty_expiration_date)" />
                            </div>
                            <div>
